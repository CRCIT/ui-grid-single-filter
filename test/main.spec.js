...
describe('Plugin SingleFilterService', function () {
  ...
  describe('La función "getPropertyWithColumnDef" ', function () {

    it('Permite obtener el valor de la propiedad de un objeto dada su nombre', function () {
       expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'propNivel1'})).toBe('p1');
    });

    it('También me permite obtener el valor para propiedades anidadas', function () {
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field: 'nivel2.propNivel2'})).toBe('p2');
    });

    it('También me permite obtener el valor para propiedades anidadas con cualquier nivel arbitrario', function () {
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'nivel3.nivel2.propNivel3'})).toBe('p3');
    });

    it('La navegación entre las propiedades estará segura pese a la existencia de nulos', function() {
      row.nivel2 = null;
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'nivel2.propNivel2'})).toBe('');
    });

    it('para cualquier tipo de nuivel', function() {
      row.nivel3 = null;
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'nivel3.nivel2.propNivel3'})).toBe('');
    });


    it('y para propiedades nulas', function() {
      row.nivel2.propNivel2 = null;
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'nivel2.propNivel2'})).toBe(null);
    });


    it('Comprobamos las condiciones límite', function() {
      expect(SingleFilterService.getPropertyWithColumnDef(null,{field: 'nivel3.nivel2.propNivel3'})).toBe('');
      expect(SingleFilterService.getPropertyWithColumnDef(row, null)).toBe('');
    });
  ...
  describe('En cuanto a la configuración a través de ColumnDef', function () {
    ...
    it('La propiedad "filterField" en caso de existir en un elemento de la lista columnDef sirve para indicar como obtener el valor a filtrar del objeto, sobreescribiendo el indicado en field', function(){
      expect(SingleFilterService.getPropertyWithColumnDef(row,{field:'karateka', filterField:'nivel2.propNivel2'})).toBe('p2');
    });
    ...
    it('Para los casos más complejos como booleanos que se muestran tras un "cellTemplate" como textos filterField puede recibir una función que obtenga dichos textos dado el objeto entidad', function (){
      ...
      expect(SingleFilterService.getPropertyWithColumnDef(row,
        { field:'enabled',
